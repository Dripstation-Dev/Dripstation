 ...
 * Currently not coding for protecting against rain. Does protect bloodsuckers from Sol
 * The rest don't and it just for looks.
 * Ported from https://github.com/Monkestation/Monkestation2.0/pull/7175
 */
/obj/item/umbrella
	name = "umbrella"
	desc = "A plain umbrella."
	icon = 'modular_dripstation/icons/obj/weapons/umbrellas.dmi'
	icon_state = "umbrella"
	item_state = "umbrella_closed"
	lefthand_file = 'modular_dripstation/icons/mob/inhands/umbrellas_inhand_lefthand.dmi'
	righthand_file = 'modular_dripstation/icons/mob/inhands/umbrellas_inhand_righthand.dmi'
	force = 5
	throwforce = 5
...
/obj/item/umbrella/worn_overlays(mutable_appearance/standing, isinhands)
	...
	if(open)
		if(ISODD(holder.get_held_index_of_item(src))) //left hand or right hand?
			. += mutable_appearance(lefthand_file, item_state + "_BACK", BELOW_MOB_LAYER)
		else
			. += mutable_appearance(righthand_file, item_state + "_BACK", BELOW_MOB_LAYER)


/obj/item/umbrella/proc/on_transform(obj/item/source, mob/user, active)
	SIGNAL_HANDLER
	item_state = active ? on_item_state : item_state
	open = active
	if(user)
...
/*
/obj/item/umbrella/get_held_offsets(isinhands)
	. = ..()
	var/mob/holder = loc
	if(open)
		.[2] += open_y_offset
...
/obj/item/umbrella/parasol
	...
	icon_state = "parasol"
	item_state = "parasol_closed"
	on_item_state = "parasol_on"
	random_color = FALSE
