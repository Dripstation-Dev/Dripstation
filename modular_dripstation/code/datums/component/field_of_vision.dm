/*
//ported from Escape From Nevada, all credits to them
...
/datum/component/field_of_vision/RegisterWithParent()
	...
	RegisterSignal(source, COMSIG_MOB_FOV_VIEW, .proc/in_fow_view)
...
/datum/component/field_of_vision/proc/generate_fov_holder(mob/living/source, shadow_angle = FOV_90_DEGREES, angle = 0, register = TRUE, delete_holder = FALSE)
	...
	if(QDELETED(fov_holder))
		...
		shadow_mask = image('modular_dripstation/icons/hud/fov_15x15.dmi', fov_holder, "[shadow_angle]")
		shadow_mask.plane = FIELD_OF_VISION_BLOCKER_PLANE//FIELD_OF_VISION_MASK_PLANE
		...
		visual_shadow = image('modular_dripstation/icons/hud/fov_15x15.dmi', fov_holder, "[shadow_angle]")
		...
		if(has_adj_mask)
			adj_mask = image('modular_dripstation/icons/hud/fov_15x15.dmi', fov_holder, "adj_mask", FIELD_OF_VISION_MASK_LAYER)
...
/datum/component/field_of_vision/proc/resize_fov(list/old_view, list/new_view)
	...
	for(var/direction in GLOB.cardinals)
		var/image/mask_extension = image('modular_dripstation/icons/hud/fov_15x15.dmi', "darkness")
		...
		var/image/visual_extension = image('modular_dripstation/icons/hud/fov_15x15.dmi', "darkness")
...
/datum/component/field_of_vision/proc/object_permanence_update()
	...
	if(HAS_TRAIT(parent_mob, TRAIT_BLIND))
		return
	//var/has_alpha = visual_shadow?.alpha//fov_holder?.alpha
	for(var/mob/visible_mob in view(world.view, parent_mob))
		...
		if(!parent_mob.in_fow(visible_mob, TRUE))
			continue
		var/datum/weakref/mob_ref = WEAKREF(visible_mob)
		var/image/fov_image/inverse/ghost = new()
		ghost.loc = mob_turf
		ghost.icon_state = "whoswatchingme"
		...
		addtimer(CALLBACK(src, PROC_REF(remove_image), parent_mob, mob_ref, ghost), 5.5 SECONDS)//clean this mess probably

/datum/component/field_of_vision/proc/remove_image(mob/living/parent_mob, datum/weakref/mob_ref, image/ghost)
	parent_mob?.client?.images -= object_permanence_images[mob_ref]
	qdel(ghost)
...
/datum/component/field_of_vision/proc/on_examinate(mob/living/source, atom/target)
	SIGNAL_HANDLER
	if(visual_shadow?.alpha)//if(fov_holder?.alpha)
...
/datum/component/field_of_vision/proc/in_fow_view(mob/living/source, atom/center, dist, list/viewed_list)
	SIGNAL_HANDLER
	if(fov_holder?.alpha)
...
/datum/component/field_of_vision/proc/set_fov_angle(new_angle)
	fov_angle = new_angle
...
/datum/component/field_of_vision/proc/update_mask()
	SIGNAL_HANDLER
	var/mob/parent_mob = parent
	var/client/parent_client = parent_mob.client
	if(!parent_client) //Love client volatility!!
...
/datum/component/field_of_vision/RegisterWithParent()
	...
	//RegisterSignal(parent, COMSIG_MOB_VISIBLE_MESSAGE, .proc/on_visible_message)
	//RegisterSignal(parent, COMSIG_MOB_EXAMINATE, .proc/on_examinate)
	SSfield_of_vision.processing |= src

/datum/component/field_of_vision/UnregisterFromParent()
	. = ..()
	UnregisterSignal(parent, list(COMSIG_MOB_RESET_PERSPECTIVE, COMSIG_MOB_CLIENT_CHANGE_VIEW, COMSIG_ATOM_DIR_CHANGE, COMSIG_LIVING_DEATH, COMSIG_LIVING_REVIVE, COMSIG_MOB_LOGOUT))
	SSfield_of_vision.processing -= src
