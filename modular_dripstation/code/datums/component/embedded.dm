...
/datum/component/embedded/process(seconds_per_tick)
	...
	var/pain_chance_current = DT_PROB_RATE(pain_chance / 100, seconds_per_tick) * 100
	...
	var/fall_chance_current = DT_PROB_RATE(fall_chance / 100, seconds_per_tick) * 100
...
/datum/component/embedded/proc/fallOut()
	...
	if(harmful)
		...
		limb.receive_damage(brute=(1-pain_stam_pct) * damage, sharpness=SHARP_EDGED, wound_bonus = 5)
		victim.adjustStaminaLoss(pain_stam_pct * damage)
	if(victim)
		...
		weapon.visible_message(span_danger("[weapon] falls [harmful ? "out" : "off"] of [limb.name]!"))
	safeRemove()
...
/datum/component/embedded/proc/complete_rip_out(mob/living/carbon/riper, mob/living/carbon/victim, obj/item/I, obj/item/bodypart/limb, time_taken)
	...
	if(harmful)
		...
		victim.adjustStaminaLoss(pain_stam_pct * damage)
		victim.flick_pain(100, TRUE, TRUE)
...
/datum/component/embedded/proc/checkTweeze(mob/living/carbon/victim, obj/item/possible_tweezers, mob/user)
	...
	if(ishuman(victim)) // check to see if the limb is actually exposed
		...
		if(!victim_human.can_inject(user, limb.body_zone, FALSE))
			return TRUE
