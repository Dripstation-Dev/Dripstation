...
/datum/round_event_control/earthquake/canSpawnEvent(players_amt, allow_magic)
	. = ..()
	if(!.)
		return .
...
/datum/round_event/earthquake/tick()
		...
		if(ISMULTIPLE(activeFor, 10))
			for(var/mob/earthquake_witness as anything in GLOB.player_list)
				...
				earthquake_witness.playsound_local(
					earthquake_witness,
					pick(
						'modular_dripstation/sound/misc/earth_rumble_distant1.ogg',
						'modular_dripstation/sound/misc/earth_rumble_distant2.ogg',
						'modular_dripstation/sound/misc/earth_rumble_distant3.ogg',
						'modular_dripstation/sound/misc/earth_rumble_distant4.ogg',
					),
					75,
				)
	...
	if(activeFor == endWhen - 1)
		...
		for(var/turf/turf_to_quake in edges)
			turf_to_quake.Shake(pixelshiftx = 0.5, pixelshifty = 0.5, duration = 1 SECONDS)
		playsound(epicenter, 'modular_dripstation/sound/misc/metal_creak.ogg', 125, TRUE)

/datum/round_event/earthquake/end()
	playsound(epicenter, 'modular_dripstation/sound/misc/earth_rumble.ogg', 125)
	for(var/mob/earthquake_witness as anything in GLOB.player_list)
	...
	for(var/turf/turf_to_shred in turfs_to_shred)
		...
		if(istype(turf_to_shred.baseturfs, /turf/open/floor/plating/asteroid) && prob(95))
