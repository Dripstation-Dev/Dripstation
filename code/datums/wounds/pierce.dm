...
/datum/wound/pierce
	...
	var/external_bleeding_chance
	/// If we let off blood when hit, the max blood lost is this * the incoming damage
	var/external_bleeding_coefficient
...
/datum/wound/pierce/receive_damage(wounding_type, wounding_dmg, wound_bonus)
	if(victim.stat == DEAD || wounding_dmg < 5)
		return
	if(victim.blood_volume && prob(external_bleeding_chance + wounding_dmg))
		if(limb.current_gauze && limb.current_gauze.splint_factor)
			wounding_dmg *= (1 - limb.current_gauze.splint_factor)
		var/blood_bled = rand(1, wounding_dmg * external_bleeding_coefficient) // 12 brute toolbox can cause up to 15/18/21 bloodloss on mod/sev/crit
		switch(blood_bled)
			if(1 to 6)
...
/datum/wound/pierce/moderate
	name = "Minor Breakage"
	desc = "Patient's skin has been broken open, causing severe bruising and minor bleeding in affected area."
	...
	external_bleeding_chance = 30
	external_bleeding_coefficient = 1.25
...
/datum/wound/pierce/severe
	name = "Open Puncture"
	desc = "Patient's internal tissue is penetrated, causing sizeable bleeding and reduced limb stability."
	...
	external_bleeding_chance = 60
	external_bleeding_coefficient = 1.5
...
/datum/wound/pierce/critical
	...
	desc = "Patient's internal tissue and circulatory system is shredded, causing significant bleeding and damage to internal organs."
	...
	external_bleeding_chance = 80
	external_bleeding_coefficient = 1.75
