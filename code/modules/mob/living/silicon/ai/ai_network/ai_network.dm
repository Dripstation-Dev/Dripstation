...
/datum/ai_network/process()
	...
	if(local_cpu_usage[AI_WEAPONRY_RESEARCH])
		var/points = max(round(AI_RESEARCH_PER_CPU * (local_cpu_usage[AI_WEAPONRY_RESEARCH] * total_cpu * resources_assigned)), 0)
		points = clamp(points * AI_RESEARCH_WEAPONRY_POINT_MULTIPLIER, 0, MAX_AI_WEAPONRY_RESEARCH_PER_TICK)
		SSresearch.science_tech.add_point_list(list(TECHWEB_POINT_TYPE_WEAPONRY = points))

	if(local_cpu_usage[AI_MEDICAL_RESEARCH])
		var/points = max(round(AI_RESEARCH_PER_CPU * (local_cpu_usage[AI_MEDICAL_RESEARCH] * total_cpu * resources_assigned)), 0)
		points = clamp(points * AI_RESEARCH_MEDICAL_POINT_MULTIPLIER, 0, MAX_AI_MEDICAL_RESEARCH_PER_TICK)
